...
export class ViewOrdersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get('http://172.17.7.104:8000/api/orders/', { headers }).subscribe(
      (response: any) => {
  ...
  fetchProductDetails(product_ids: string[], orderData: any): void {
    if (product_ids.length > 0) {
      ...
      product_ids.forEach((productId, index) => {
        this.http.get(`http://172.17.7.104:8000/api/Product_Table/${productId}`, { headers }).subscribe(
          (response: any) => {
  ...
  fetchShippingDetails(shipping_id: number, orderData: any): void {
    ...
    this.http.get(`http://172.17.7.104:8000/api/shipping/${shipping_id}`, { headers }).subscribe(
      (response: any) => {
  ...
  cancelOrder(order: any, orderIndex: number): void {
    ...
    this.http.patch(`http://172.17.7.104:8000/api/shipping/${order.shipping_id}/`, 
